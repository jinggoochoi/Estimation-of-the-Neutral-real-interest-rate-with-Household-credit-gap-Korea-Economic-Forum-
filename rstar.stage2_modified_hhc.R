...
##------------------------------------------------------------------------------##
rstar.stage2_modified_hhc <- function(log.output,
                                  inflation,
  ...
  initial.parameters <- c(b.is, 1, b.ph[1:2], b.ph[4:5], b.hc, s.is, s.ph, s.hc, 0.5)
  ...
  if (!is.na(b.y.constraint)) {
    ...
    if (initial.parameters[9] < b.y.constraint) {
      initial.parameters[9] <- b.y.constraint
    }
    theta.lb[9] <- b.y.constraint
  }
  ...
  f <- function(theta) {return(-log.likelihood.wrapper_hhc(theta, y.data, x.data, stage, lambda.g, NA, xi.00, P.00)$ll.cum)}
  ...
  log.likelihood <- log.likelihood.wrapper_hhc(theta, y.data, x.data, stage, lambda.g, NA, xi.00, P.00)$ll.cum
  
  ## Get state vectors (xi.tt, xi.ttm1, xi.tT, P.tt, P.ttm1, P.tT) via Kalman filter
  states <- kalman.states.wrapper_hhc(theta, y.data, x.data, stage, lambda.g, NA, xi.00, P.00)
