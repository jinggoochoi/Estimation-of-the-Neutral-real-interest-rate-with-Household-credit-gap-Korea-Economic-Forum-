...
#lambda.g <- lambda.g.series[2]
lambda.g <- (lambda.g.series[1] + lambda.g.series[2] + lambda.g.series[3]) / 3

#lambda.g = 0.1364506 #(including rp, sample period: 1990Q3~2023Q4)
...
#lambda.z <- lambda.z.series[2]
lambda.z <- (lambda.z.series[1] + lambda.z.series[2] + lambda.z.series[3]) / 3

##lambda.z = 0 / 0 / 0.03194714 #(including rp, sample period: 1990Q3~2023Q4)
...
out.stage3 <- rstar.stage3_modified_hhc(log.output,
                           inflation,
                           output.gap,
                           relative.import.price.inflation,
                           real.interest.rate,
...
## Save output to CSV
output.kr <- format.output_hhc(out.stage3, one.sided.est, two.sided.est, real.interest.rate, sample.start, sample.end, run.se = run.se)
#write.table(output.kr, 'LW_output_220526_hhc.csv', col.names = output.col.names, quote=FALSE, row.names=FALSE, sep = ',', na = '')
write.table(output.kr, 'LW_output_220904_hhc.csv', quote=FALSE, row.names=FALSE, sep = ',', na = '')