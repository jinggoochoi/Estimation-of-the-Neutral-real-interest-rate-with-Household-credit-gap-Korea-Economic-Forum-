...
##------------------------------------------------------------------------------##
kalman.standard.errors_hhc <- function(T, states, theta, y.data, x.data, stage,
    ...
    log.likelihood.estimated.vector <- log.likelihood.wrapper_hhc(theta, y.data, x.data, stage = 3,lambda.g=lambda.g, lambda.z=lambda.z, xi.00=xi.00, P.00=P.00)$ll.vec
    ...
    for (i in 1:n.params){
        ...
        likelihood.gradient[,i] <-  (log.likelihood.wrapper_hhc(d.theta, y.data, x.data, stage = 3,lambda.g=lambda.g, lambda.z=lambda.z, xi.00=xi.00, P.00=P.00)$ll.vec - 
    ...
    for (i in 1:niter) {
      theta.i <- (hh %*% rnorm(n.params) + theta)[,1] 
          ...
          states.i <- kalman.states.wrapper_hhc(theta.i, y.data, x.data, stage, lambda.g, lambda.z, xi.00.i, pp1)
